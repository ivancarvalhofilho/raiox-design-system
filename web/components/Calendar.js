...
const Calendar = (props) => {
  ...
  useEffect(() => {
    if (props.month != null && props.month >= 0 && props.month < 12) {
      const numberOfDaysUntil = Array.from(
        Array(
      ...
      const firstDay = dayjs()
        ...
        .startOf('month')
        .weekday()
      let array = numberOfDaysUntil
        .splice(numberOfDaysUntil.length - firstDay)
        .concat(Array.from(Array(numberOfDays).keys()).map((v) => v + 1))
      if (35 - array.length > 0) {
        array = array.concat(
          Array.from(Array(35 - array.length).keys()).map((v) => v + 1),
        )
      } else {
        array = array.concat(
          Array.from(Array(42 - array.length).keys()).map((v) => v + 1),
        )
      }
      setState(array)
    }
  ...
  return (
    <CalendarContainer>
      {array && array.map((day) => <CalendarCell day={day} />)}
    </CalendarContainer>
  )
}
